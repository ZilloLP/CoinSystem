...
import de.zillolp.coinsystem.events.PlayerRemoveCoins;
...
public class RemoveCoinsCommand implements TabExecutor {
                ...
                if (args.length == 2) {
                    ...
                    if (Bukkit.getOnlinePlayers().contains(target)) {
                        if (StringUtil.isNumber(args[1])) {
                            ...
                            playerProfil.removeCoins(coins);
                            if (player != target) {
                                player.sendMessage(LanguageTools.getREMOVE_COINS_PLAYER(target, coins));
                                target.sendMessage(LanguageTools.getREMOVE_COINS_TARGET(player, coins));
                            } else {
                                player.sendMessage(LanguageTools.getREMOVE_COINS_OWN(coins));
                            }
                            Bukkit.getServer().getPluginManager().callEvent(new PlayerRemoveCoins(player, target, coins));
                        } else {
